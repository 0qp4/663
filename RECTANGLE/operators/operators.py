...
class CastingOperator(Operator):
    def __init__(self, input_vars, output_vars, ID = None):
        if sum([input_vars[i].bitsize for i in range(len(input_vars))]) != sum([output_vars[i].bitsize for i in range(len(output_vars))]):
            raise Exception("CastingOperator: the total input size does not match the total output size")
    ...
    def generate_model(self, model_type='sat'):
        ...
        if model_type in ['sat', 'milp']:
            if model_type == "sat" and self.model_version in [self.__class__.__name__ + "_XORDIFF"]:
                ...
                var_in = self.get_var_model("in", 0, bitwise=False)
                var_out1 = self.get_var_model("out", 0, bitwise=False)
                var_out2 = self.get_var_model("out", 1, bitwise=False)
